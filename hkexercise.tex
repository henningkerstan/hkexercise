...
\lstset{%
  ...
  emph={%
    ...
    exercise,%
    currentexerciseparttitle,%
    currentexercisepartpoints,%
    currentexercisepartnumber,%
    exercisepart,%
    bonuspoints,%
    getbonuspoints,%
    customlayout,%
...
\subsection{Exerciseparts}
If you have an exercise with many different parts (for example, if you have one overall topic with several areas), you can use the exercisepart environment to structure your exercise. Exerciseparts work exactly as exercises, but can only be used within an exercise environment. The following is a compiled example of such an exercise.

\begin{exercise}[Calculus]
  \vspace{-1em}
  \begin{exercisepart}[Derivatives]
    ...
    \end{enumerate}
  \end{exercisepart}\vspace{-1em}
  \begin{exercisepart}[Maxima and Minima]
...
\noindent Note that it is only possible to use the exercisepart environment within an exercise environment and it is not possible to nest exercisepart environments.
...
\end{lstlisting}
which disables the built in exercise style. Note that in this case your document will only compile exercises or exerciseparts if you have defined the respective layout. Otherwise compilation will stop with a package error message.
...
\noindent For exerciseparts you have essentially the same commands, i.e., you have the commands 
\begin{itemize}
\item \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash AtBeginExercisePart\{...\}},
\item \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash AtEndExercisePart\{...\}}, 
\item \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash currentexercisepartnumber}, 
\item \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash currentexercisepartpoints} and 
\item \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash currentexerciseparttitle}
\end{itemize}
which work as their counterparts for exercises.

\subsection{Example Layout Using (Sub-)Sections}
One option for a custom layout could be to use sections for exercises and subsections for exerciseparts. This can be achieved using the following code.
...
\AtBeginExercisepart{%
  \subsection{\texorpdfstring{%
    \currentexerciseparttitle\hfill%
    (\currentexercisepartpoints~Points)}{%
    \currentexerciseparttitle %
    (\currentexercisepartpoints~Points)}%
  }
}
...
\begin{description}
\item[2017/06/27 v1.1.0] This version introduces exerciseparts. Moreover several checks were added to avoid use of commands like \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash currentexercisepoints} outside of exercise environments. New option \textcolor{NavyBlue}{\ttfamily\bfseries customlayout} disables built in exercise layout. Added \textcolor{NavyBlue}{\ttfamily\bfseries\textbackslash bonuspoint} commands.
